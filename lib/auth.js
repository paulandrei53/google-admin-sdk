...
class GoogleAdminAuth {
    constructor(options) {
        ...
        if (!this.options.token) {
            throw new Error('Must initialize GoogleAPI with token info');
        }
        if (!this.options.token.refresh && !this.options.token.access) {
            throw new Error('Must provide either a refresh token or an access token');
        }
        // client secret needed for auto-refresh
        if (!!this.options.token.refresh && (!(this.options.client != null ? this.options.client.id : undefined) || !(this.options.client != null ? this.options.client.secret : undefined))) {
            throw new Error('If providing a refresh token, must provide client id and secret');
        }
    }
        ...
        this.request_token = (options, cb) => {
            ...
            return operation.attempt(() => {
                return quest(options, (err, resp, body) => {
                    if (!operation.retry(GoogleAdminAuth.retry_err(err, resp))) {
    ...
    request_refresh_token = (cb) => {
        ...
        return operation.attempt(() => {
            return quest(options, (err, resp, body) => {
                if (operation.retry(GoogleAdminAuth.retry_err(err, resp))) { return; }
                ...
                return (GoogleAdminAuth.response_handler(cb))(err, resp, body);
            });
        });
    }
...
GoogleAdminAuth.initClass();

module.exports = GoogleAdminAuth;
